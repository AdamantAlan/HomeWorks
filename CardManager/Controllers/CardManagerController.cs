...
namespace CardManager.Controllers
{
    ...
    public class CardManagerController : ControllerBase
    {
        ...
        public async Task<ActionResult<ResultApi<CardReadDto>>> AddCard(long userId, CardWriteDto cardWriteDto)
        {
            ...
            var id = await _mediator.Send(new CreateCardAsyncCommand { Card = _map.Map<CardModel>(cardWriteDto) });

            var cardReadDto = _map.Map<CardReadDto>(await _manager.GetAsync<CardModel>(id));
